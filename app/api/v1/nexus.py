...
async def force_sync(
    ...
    try:
        ...
        logger.info(f"✓ Sync with park_name completed: {result.stdout[:200]}")
        
        return {
            "status": "success",
            "message": "Синхронизация транзакций с маркировкой park_name завершена",
            "output": result.stdout[:500] if result.stdout else "Успешно",
            "timestamp": datetime.now().isoformat()
        }
...
async def telegram_webhook(request: Request):
    ...
    Принимает updates от Telegram API
    """
    try:
    ...
    try:
        ...
        return HTMLResponse(
            ...
            <div class="widget-error">
                ...
                <div class="error-detail">{str(e)[:100]}</div>
            </div>
            """,
            status_code=200
        )
...
async def live_profit(
    ...
    try:
        ...
        return HTMLResponse(
            content=f'<div style="padding: 20px; color: #ff5252;">⚠️ Ошибка загрузки прибыли: {str(e)}</div>',
            status_code=200
        )
