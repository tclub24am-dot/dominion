...
class User(Base):
    ...
    tenant_id = Column(String, default="s-global", index=True)  # SaaS Tenant ID (изоляция данных)
    username = Column(String, unique=True, index=True)
    hashed_password = Column(String)
    full_name = Column(String)
    photo_url = Column(String, nullable=True)
    telegram_id = Column(String, nullable=True)  # Telegram ID для уведомлений
    phone = Column(String, nullable=True, index=True)  # Основной номер телефона
    role = Column(
    ...
    # HR-МЕТРИКИ ЛОГИСТА (Протокол LOGIST-PAY v200.15)
    base_salary = Column(Float, default=130000.0)       # Базовый оклад (₽/мес)
    kpi_bonus_ratio = Column(Float, default=0.0)        # KPI-бонус (коэффициент 0.0-1.0)
    m4_fines_count = Column(Integer, default=0)         # Счётчик штрафов М4 за текущий месяц
...
class CallLog(Base):
    """
    S-АТС: Лог звонков v200.15
    Поддерживает: 1ATS, Asterisk, собственная S-АТС
    Tenant Hard Isolation: каждый звонок жёстко привязан к tenant_id
    VERSHINA v200.15: asterisk_unique_id — unique=True для атомарной дедупликации
    Race Condition Fix: IntegrityError вместо SELECT-проверки в telephony.py
    """
    __tablename__ = "call_logs"
    __table_args__ = (
        # КРИТИЧНЫЙ составной индекс: мгновенный поиск водителя при входящем звонке
        Index('ix_call_logs_tenant_caller', 'tenant_id', 'caller_phone'),
        # Индекс для сортировки истории звонков
        Index('ix_call_logs_timestamp', 'timestamp'),
        # VERSHINA v200.16.3: unique-индекс ix_call_logs_asterisk_uid УДАЛЁН —
        # уникальность обеспечивается через Column(unique=True) на asterisk_unique_id.
        # Дублирование индексов устранено для чистоты схемы PostgreSQL.
        {'extend_existing': True}
    )

    id                 = Column(Integer, primary_key=True, index=True)
    tenant_id          = Column(String(100), nullable=False,
                                default="s-global", index=True)   # SaaS-изоляция (ОБЯЗАТЕЛЬНО)
    user_id            = Column(Integer, ForeignKey("users.id"), nullable=True)  # Связь с водителем
    caller_phone       = Column(String(20), nullable=True)        # Номер звонящего
    callee_phone       = Column(String(20), nullable=True)        # Номер принимающего (S-АТС)
    call_status        = Column(String(20), default="unknown")    # answered/missed/ended/new/unknown
    duration           = Column(Integer, default=0)               # Длительность в секундах
    recording_url      = Column(Text, nullable=True)              # URL записи разговора (S-АТС)
    ai_rating          = Column(Integer, default=5)               # AI-оценка звонка
    timestamp          = Column(DateTime, default=dt.datetime.now, index=True)  # Время звонка
    # VERSHINA v200.15: Уникальный ID звонка от Asterisk (для дедупликации событий)
    # unique=True + nullable=True: NULL не нарушает уникальность (стандарт SQL)
    # Формат: "<epoch>.<channel_seq>" (например: "1709935200.42")
    asterisk_unique_id = Column(String(64), nullable=True, unique=True)  # Asterisk UniqueID
