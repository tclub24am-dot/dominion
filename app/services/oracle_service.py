...
class OracleService:
    ...
    def __init__(self):
        ...
        if self.api_key and self.api_key != "pending_configure_in_google_ai_studio":
            ...
            # IRON SHIELD v200.16: НЕ вызываем create_task() в __init__ —
            # event loop может быть недоступен при импорте модуля.
            # Вызов _check_handshake() перенесён в lifespan FastAPI (main.py).
            logger.info("  Handshake will be performed on application startup (lifespan).")
        else:
    ...
    async def _check_handshake(self):
        """Проверка связи с Ollama при старте (async, не блокирует event loop)"""
        try:
            # Шаг 1: Проверяем доступность Ollama API через httpx.AsyncClient
            ...
            # Шаг 2: Проверяем наличие модели через asyncio.create_subprocess_exec
            try:
                ...
                if proc.returncode != 0:
                    logger.warning(f"ollama list failed: {stderr.decode()}")
                ...
                if model_base_name in models_output or self.model in models_output:
                    ...
                    logger.info(f"  ✓ API Key: [CONFIGURED]")  # IRON SHIELD: ключ не логируется
