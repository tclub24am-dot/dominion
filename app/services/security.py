...
async def get_current_user_from_cookie(
    ...
    try:
        ...
        if request_tenant_id and user.tenant_id != request_tenant_id:
            logger.warning(
                "Tenant mismatch detected for authenticated cookie session",
                extra={"username": user.username, "request_tenant_id": request_tenant_id, "user_tenant_id": user.tenant_id},
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Доступ к чужому tenant запрещён."
            )
