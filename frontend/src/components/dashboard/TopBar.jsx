...
export default function TopBar({ theme, onToggleTheme }) {
  ...
  return (
    <motion.header
      className={`
        relative z-40 flex items-center justify-between px-6 py-3
        border-b backdrop-blur-xl
        ${isDark
          ? 'bg-dominion-deep/80 border-dominion-border'
          : 'bg-ivory-card/80 border-ivory-border'
        }
      `}
      ...
      <div className="flex items-center gap-4">
        ...
        <motion.div
          className={`
            ...
            ${isDark
              ? 'bg-gradient-to-br from-dominion-gold/20 to-dominion-gold/5 border border-dominion-gold/30'
              : 'bg-gradient-to-br from-ivory-gold/20 to-ivory-gold/5 border border-ivory-gold/30'
            }
          `}
        ...
        >
          <Shield
            size={18}
            className={isDark ? 'text-dominion-gold' : 'text-ivory-gold'}
          />
        </motion.div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <h1
            className={`
              ...
              ${isDark ? 'text-gold-gradient' : 'text-ivory-gold'}
            `}
            style={isDark ? {
          ...
          {/* Разделитель */}
          <div className={`w-px h-5 ${isDark ? 'bg-dominion-border' : 'bg-ivory-border'}`} />
      ...
      <div className="flex items-center gap-4">
        ...
        <motion.button
          ...
          className={`
            ...
            ${isDark
              ? 'bg-white/5 border border-white/10 hover:border-dominion-gold/40 text-dominion-gold'
              : 'bg-ivory-gold/10 border border-ivory-border hover:border-ivory-gold/40 text-ivory-gold'
            }
          `}
          whileHover={{ scale: 1.1, rotate: 15 }}
          whileTap={{ scale: 0.9 }}
          title={isDark ? 'Переключить на Ivory тему' : 'Переключить на Dark тему'}
        >
